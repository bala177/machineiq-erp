...
export function SettingsOverview({ onSelect }: { onSelect: (tab: string) => void }) {
  ...
  return <div className="space-y-6">
    ...
    {filtered.map(group => <section key={group.title} aria-label={group.title} className="space-y-3"><div><h3 className="font-semibold text-fg">{group.title}</h3><p className="text-sm text-fg-muted">{group.description}</p></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{group.entries.map(entry => {
      ...
      const section = 'section' in entry && entry.section ? `?section=${entry.section}` : '';
      return 'href' in entry ? <Link key={entry.title} href={`${entry.href}${section}`} className={style}>{content}</Link> : <a key={entry.title} href={`/admin/settings?tab=${entry.tab}`} className={style} onClick={event => { if (!event.ctrlKey && !event.metaKey && !event.shiftKey && !event.altKey) { event.preventDefault(); onSelect(entry.tab!); } }}>{content}</a>;
    })}</div></section>)}
  </div>;
}
