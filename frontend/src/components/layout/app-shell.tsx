...
import {
  ...
  Bell, Users, Settings, Menu, X, LogOut, Search,
  Sun, Moon, ChevronDown, UserCircle,
  HelpCircle, PanelLeftClose, PanelLeftOpen,
  ChevronsLeft, Sparkles, Info, Boxes, Truck,
} from 'lucide-react';
...
function RoleChip({ role }: { role: string }) {
  return (
    <span className={clsx('inline-flex items-center rounded px-1.5 py-0.5 text-[11px] font-semibold capitalize', roleColor(role))}>
      {roleLabel(role)}
    </span>
  );
}
    ...
    <div className="flex items-center gap-2.5 min-w-0">
      ...
      {!collapsed && (
        <div className="min-w-0">
          ...
          <p className="mt-0.5 truncate text-[10px] font-medium uppercase tracking-widest text-fg-muted">
            ERP Platform
          </p>
        </div>
      )}
...
function ProfileDropdown({
  user, theme, setTheme, logout, onClose,
}: {
  user: any; theme: string; setTheme: (t: any) => void; logout: () => void; onClose: () => void;
}) {
  return (
    <div
      className={clsx(
        'absolute right-0 top-[calc(100%+8px)] z-[120] w-[300px] rounded-xl border border-border bg-surface shadow-xl',
        'ring-1 ring-black/5 dark:ring-white/5',
      )}
      style={{ animation: 'dropdown-in 120ms ease-out' }}
    >
      {/* User header */}
      <div className="px-4 py-4">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-brand-600 text-sm font-bold text-white shadow">
      ...
      <div className="px-1.5 py-1.5">
        <p className="px-2.5 pb-1 pt-0.5 text-[10px] font-bold uppercase tracking-widest text-fg-muted">
          Account
        </p>
        {[
          ...
          <Link key={label} href={href} onClick={onClose}
            className="flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-sm text-fg-secondary transition-colors hover:bg-surface-secondary hover:text-fg">
      ...
      <div className="px-1.5 py-1.5">
        <p className="px-2.5 pb-2 pt-0.5 text-[10px] font-bold uppercase tracking-widest text-fg-muted">
          Appearance
        </p>
        <div className="flex items-center gap-1 rounded-lg border border-border bg-surface-secondary p-1">
          {themeOptions.map((opt) => (
            <button key={opt.value} onClick={() => setTheme(opt.value)}
              className={clsx(
                'flex flex-1 items-center justify-center gap-1.5 rounded-md py-1.5 text-xs font-medium transition-all duration-150',
                theme === opt.value ? 'bg-surface text-fg shadow-sm' : 'text-fg-muted hover:text-fg-secondary',
              )}>
  ...
  return (
    ...
    >
      ...
      <item.icon className={clsx(
        ...
        active ? 'text-brand-600 dark:text-brand-400' : 'text-fg-tertiary group-hover:text-fg-secondary',
      )} />
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-surface-secondary dark:bg-[#0b0f19]">
      ...
      >
        {/* Brand */}
        <div className={clsx(
          'flex h-[58px] shrink-0 items-center justify-between border-b border-border px-4',
          ...
          <button
            ...
            className="hidden md:flex items-center justify-center rounded-md p-1.5 text-fg-muted hover:bg-surface-secondary hover:text-fg transition-colors"
          >
            {collapsed
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="flex rounded-md p-1.5 text-fg-muted hover:bg-surface-secondary hover:text-fg md:hidden"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <nav className="sidebar-scroll flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
          ...
          {/* Core */}
          {!collapsed && (
            <p className="mb-1.5 px-3 text-[10px] font-bold uppercase tracking-[0.15em] text-fg-muted">
              Core
            </p>
          )}
      ...
      <div className="flex flex-1 flex-col overflow-hidden min-w-0">
        ...
        <header className="relative z-[100] flex h-[58px] shrink-0 items-center justify-between border-b border-border bg-surface/95 backdrop-blur-sm px-4 lg:px-5 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center justify-center rounded-md p-2 text-fg-muted hover:bg-surface-secondary hover:text-fg md:hidden"
            >
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Link
              href="/notifications"
              className="relative flex h-9 w-9 items-center justify-center rounded-lg text-fg-muted transition-colors hover:bg-surface-secondary hover:text-fg-secondary"
            >
            ...
            <div ref={profileRef} className="relative">
              <button
                ...
                className={clsx(
                  ...
                  profileOpen
                    ? 'bg-surface-secondary shadow-sm ring-1 ring-border-strong'
                    : 'hover:bg-surface-secondary',
                )}
              >
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-brand-600 text-[11px] font-bold text-white shadow-sm">
                  {initials(user?.firstName, user?.lastName)}
                </div>
                <div className="hidden text-left sm:block">
                  ...
                  <p className="mt-0.5 text-[11px] text-fg-muted capitalize leading-none">
                    {roleLabel(user?.role ?? '')}
                  </p>
                </div>
                <ChevronDown className={clsx(
                  'hidden h-3.5 w-3.5 text-fg-muted transition-transform duration-150 sm:block',
        ...
        <main className="relative z-0 flex-1 overflow-y-auto">
          <div className="mx-auto max-w-7xl px-5 py-6 pb-24 md:px-6 md:py-7 lg:px-8 lg:py-8">
            {children}
          </div>
        </main>
      </div>
            ...
            return (
              <Link
                ...
                className={clsx(
                  ...
                  active ? 'text-brand-600' : 'text-fg-muted',
                )}
              >
