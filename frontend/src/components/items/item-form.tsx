    ...
    <div className="min-h-[400px] space-y-5 p-6">
      ...
      {activeTab === 'Sales & Purchase' && <div className="grid gap-5 lg:grid-cols-2">
        <section className="rounded-xl border border-border p-4">
          ...
          {form.salesEnabled && <div className="mt-4 space-y-4">
            <label><FieldLabel as="span" tooltip="Default price per base unit. It can be changed on a transaction.">Selling price</FieldLabel><input className={fc('sellingPrice')} type="number" min="0" step="0.01" inputMode="decimal" value={form.sellingPrice} onChange={(event) => set('sellingPrice', event.target.value)} placeholder="0.00" />{err('sellingPrice')}</label>
      ...
      {activeTab === 'Inventory & Tax' && <div className="space-y-5">
        ...
        {!isService && <section>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label><FieldLabel as="span" tooltip="Part number assigned by the original equipment manufacturer.">Manufacturer part number</FieldLabel><input className="input-field" value={form.manufacturerPartNumber} onChange={(event) => set('manufacturerPartNumber', event.target.value)} placeholder="e.g. 6SL3210-1KE15-8UF2" /></label>
