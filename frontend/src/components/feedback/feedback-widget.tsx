...
const issueChoices: Record<FeedbackType, string[]> = {
  broken: ['Cannot save changes', 'Page does not load', 'Button does not respond', 'Information is incorrect'],
  hard_to_use: ['Cannot find what I need', 'Too many steps', 'Instructions are unclear', 'Layout is difficult to use'],
  suggestion: ['Reduce repeated entry', 'Add a missing option', 'Improve search or filters', 'Make this easier on mobile'],
  general: ['I need help with this page', 'I have a question about this workflow'],
  positive: ['Easy to use', 'Saves me time', 'The information is clear'],
};
...
export function FeedbackWidget({ enabled, onSubmitted }: { enabled: boolean; onSubmitted?: () => void }) {
  ...
  const currentSection = pathname.startsWith('/sales')
    ? searchParams.get('tab') === 'reports' ? 'Sales reports' : ({ enquiry: 'Enquiries', quote: 'Quotations', order: 'Sales orders', project: 'Machine projects' } as Record<string, string>)[searchParams.get('kind') || ''] || 'Sales & machine projects'
    : sectionForPath(pathname)?.label || 'This page';
  const reportSection = section || currentSection;
  const choices = type === 'broken' && /Sales|Quotations|Enquiries|Machine projects/.test(reportSection)
    ? [...issueChoices.broken, ...(reportSection === 'Sales reports' ? ['Report totals look wrong', 'Cannot export or print'] : ['Cannot submit or approve', 'Cannot create the next document'])]
    : issueChoices[type];
  ...
  // ?feedback=1 opens the form on arrival, so a reviewer can be sent straight to the right screen.
  useEffect(() => {
  ...
  const reset = () => {
    setType('broken'); setUrgency('important'); setMessage(''); setSection(''); setIssue(''); setContactAllowed(true); setScreenshot(null); setError(''); setSent(false);
  };
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!issue && message.trim().length < 5) return setError('Choose an issue above or add a short description.');
    const summary = [`Section: ${reportSection}`, ...(issue ? [`Issue: ${issue}`] : []), ...(message.trim() ? [`Details: ${message.trim()}`] : [])].join('\n');
    setSaving(true); setError('');
    try {
      const width = window.innerWidth;
      await api.post('/feedback', {
        type, urgency, message: summary, contactAllowed, screenshotDataUrl: screenshot || undefined,
          ...
          {sent ? <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
            ...
            <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
              ...
              <fieldset><legend className="mb-2 text-sm font-semibold text-fg">What would you like to share?</legend><div className="grid grid-cols-2 gap-2">
                {types.map(({ value, icon: Icon }) => <button type="button" key={value} aria-pressed={type === value} onClick={() => { setType(value); setIssue(''); }} className={clsx('flex items-center gap-2 rounded-lg border px-3 py-2.5 text-left text-xs font-medium transition', type === value ? 'border-brand-500 bg-brand-50 text-brand-700 dark:bg-brand-950/30 dark:text-brand-300' : 'border-border text-fg-secondary hover:bg-surface-secondary')}><Icon className="h-4 w-4 shrink-0" />{feedbackTypeLabels[value]}</button>)}
              </div></fieldset>
              <fieldset><legend className="mb-2 text-sm font-semibold text-fg">{type === 'positive' ? 'What worked well?' : 'Choose a quick description'}</legend><div className="flex flex-wrap gap-2">{choices.map(choice => <button type="button" key={choice} aria-pressed={issue === choice} onClick={() => setIssue(issue === choice ? '' : choice)} className={clsx('rounded-lg border px-3 py-2 text-left text-xs font-medium', issue === choice ? 'border-brand-500 bg-brand-50 text-brand-700 dark:bg-brand-950/30 dark:text-brand-300' : 'border-border text-fg-secondary hover:bg-surface-secondary')}>{choice}</button>)}</div><p className="mt-2 text-xs text-fg-muted">Choose one and send, or add your own description below.</p></fieldset>
              <label className="block w-full"><span className="mb-2 block text-sm font-semibold text-fg">Tell us what happened {issue ? <span className="font-normal text-fg-muted">(optional details)</span> : <span className="text-red-500">*</span>}</span><textarea ref={textareaRef} rows={5} value={message} onChange={(e) => setMessage(e.target.value)} maxLength={4500} placeholder={issue ? 'Anything else that would help us? A short note is enough.' : type === 'suggestion' ? 'What would make this page easier for you?' : type === 'positive' ? 'What helped you get your work done?' : 'For example: I selected Save, but my changes did not appear.'} className="input-field block w-full min-w-0 resize-y" /><span className="mt-1 block text-right text-[11px] text-fg-muted">{message.length}/4500</span></label>
