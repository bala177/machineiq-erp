> **Historical archive — not an active scope or release authority.** Superseded by Dashboard.docx plus Product Specification v1.0. See [current baseline](../specification-baseline.md). Original content below is retained for provenance; paths, statuses and release assignments may be historical.

# MachineIQ Specification and Release Tracker

> **Baseline transition — 2026-09-07:** This tracker records the previous Dashboard.docx baseline and historical build evidence. The revised [Product Specification v1.0](specs/MachineIQ_ERP_Product_Specification_v1.0.pdf) is reviewed in the [R2 verification and implementation plan](plans/2026-09-07-release2-sales-project-initiation.md), which contains the current R2 gap matrix, R1–R10 mapping, dependencies and acceptance gates. Current packages are `2.1.0-rc.2`; the rc.1 control information below is historical. Old Done statuses are not certification against the revised PDF, and its old future-release assignments must not be used for R2 implementation.
