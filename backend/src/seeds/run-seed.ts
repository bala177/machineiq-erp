...
async function seed() {
  ...
  for (const [code, module, action] of ADMIN_PERMISSION_DEFINITIONS) {
  ...
  await companies.upsert(
    {
      code: 'MIQ',
      name: 'MachineIQ Manufacturing Pvt. Ltd.',
      email: 'operations@machineiq.com',
  ...
  await suppliers.upsert(
    {
      code: 'SUP-00001',
      name: 'Precision Motion Systems',
      contactPerson: 'Ravi Kumar',
      email: 'sales@precisionmotion.example',
      phone: '+91 80 5555 0190',
      address: 'Bengaluru',
      category: 'Motion Control',
      paymentTerms: 'Net 30',
      taxRegistrationNumber: '29AABCP0001F1Z1',
      currencyCode: 'INR',
      bankDetails: {},
...
async function seedDemo() {
  ...
  await customers.upsert(
    {
      code: 'CUS-00001',
      name: 'Atlas Beverage Systems',
      accountType: 'customer',
      customerType: 'business',
      displayName: 'Atlas Beverage',
      companySize: 'medium',
      industry: 'Food & Beverage',
      website: null,
      contactPerson: 'Nina Alvarez',
      email: 'nina@atlasbev.example',
      phone: '+1-555-0199',
      mobile: null,
      ...
      priceList: null,
      deliveryTerms: 'FCA',
      notes: 'Deterministic Release 1 review customer',
    },
    { conflictPaths: ['code'] },
  );
  ...
  if (!(await documents.findOne({ where: { domain: 'Project' } }))) {
    const project = await documents.save(
      documents.create({
        domain: 'Project',
        deletedAt: null,
        data: { projectNo: 'PRJ-2026-0001', name: 'Atlas High-Speed Packaging Cell', customerId: customer._id, projectManagerId: reviewer._id, stage: 'engineering', health: 'healthy', priority: 'high', description: 'Deterministic client-review project', teamMembers: [reviewer._id], milestones: [] },
      }),
    );
    await documents.save(documents.create({ domain: 'Machine', deletedAt: null, data: { projectId: project._id, name: 'Cartoning and Case Packing Cell', description: 'Release 1 demo machine', status: 'in_progress' } }));
    await documents.save(documents.create({ domain: 'Task', deletedAt: null, data: { projectId: project._id, title: 'Complete mechanical design review', status: 'in_progress', priority: 'high', ownerId: reviewer._id, departmentId: pmDepartment._id } }));
  }
}
