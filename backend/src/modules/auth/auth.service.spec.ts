...
describe('AuthService PostgreSQL setup', () => {
  ...
  const transactionAuditLogs = { create: jest.fn((value) => value), save: jest.fn() };
  const dataSource = {
    transaction: jest.fn(async (_level: string, work: (manager: any) => unknown) =>
      work({
        getRepository: (entity: unknown) => {
          ...
          if (entity === AuditLogEntity) return transactionAuditLogs;
  ...
  it('creates the first administrator and organization settings atomically', async () => {
    ...
    transactionPermissions.find.mockResolvedValue([{ _id: 'eef56d5c-468e-4228-a521-8cf73c2185e5', code: 'items.manage' }]);
    ...
    expect(transactionSettings.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        key: 'commercial_preferences',
        value: expect.objectContaining({ organizationName: 'Acme', machineSegment: 'SPM' }),
      }),
      { conflictPaths: ['key'] },
    );
  ...
  it('rejects setup when any user already exists', async () => {
    ...
    await expect(service.setup({ organizationName: 'Acme', email: 'admin@acme.com', password: 'SecurePass1', firstName: 'Jane', lastName: 'Doe' })).rejects.toThrow(ForbiddenException);
  });
});
